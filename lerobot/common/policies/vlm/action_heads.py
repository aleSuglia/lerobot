...
class ActionInputAdapter(nn.Module):
    def __init__(self, config):
        super().__init__()
        mlp_depth = config.action_input_adapter_mlp_depth
        modules = [nn.Linear(config.max_action_dim, config.hidden_size)]
...
class L1RegressionActionHead(nn.Module):
    ...
    def forward(self, actions_hidden_states):
        ...
        action = self.model(actions_hidden_states)
        return action
...
class DiffusionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.noise_scheduler = DDIMScheduler(
            num_train_timesteps=num_diffusion_steps, beta_schedule="squaredcos_cap_v2"
        )
